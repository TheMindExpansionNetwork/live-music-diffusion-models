...
class PreEncodedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # Filter by silence_ratio from per-stem json metadata.
        if silence_threshold is not None:
            kept = []
            missing = 0
            for fn in self.filenames:
                ext = fn.split(".")[-1]
                md_fn = fn.replace(f".{ext}", ".json")
                try:
                    ...
                    missing += 1
                    continue
                sr = md.get("silence_ratio", None)
                if sr is None:
                    missing += 1
                    continue
                if sr < silence_threshold:
                    kept.append(fn)
            print(f"Silence threshold {silence_threshold}: {len(kept)}/{len(self.filenames)} files kept "
                  f"({missing} missing/unreadable metadata).")
            self.filenames = kept

        # Stem mapping for paired-stem (accompaniment) loading.
        self.stem_mapping_path = stem_mapping_path
        self.stem_mapping = None
        self.id_to_filename = None
        if stem_mapping_path is not None:
            with open(stem_mapping_path, "r") as f:
                self.stem_mapping = json.load(f)
            # Build ID -> filename lookup over all collected files (basename without extension).
            self.id_to_filename = {
                os.path.splitext(os.path.basename(fn))[0]: fn for fn in self.filenames
            }
            # Restrict filenames to those whose ID is a mapping key AND has at least one resolvable
            # accompaniment stem present in id_to_filename. This guarantees __getitem__ can always
            # return a valid pair when stem mapping is active.
            filtered = []
            for fn in self.filenames:
                stem_id = os.path.splitext(os.path.basename(fn))[0]
                if stem_id not in self.stem_mapping:
                    continue
                accomp_ids = [aid for aid in self.stem_mapping[stem_id] if aid in self.id_to_filename]
                if len(accomp_ids) == 0:
                    continue
                filtered.append(fn)
            print(f"Stem mapping active ({stem_mapping_path}): {len(filtered)}/{len(self.filenames)} files have valid accomp pairs.")
            self.filenames = filtered
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.stem_mapping is not None:
                stem_id = os.path.splitext(os.path.basename(latent_filename))[0]
                accomp_ids = [aid for aid in self.stem_mapping[stem_id] if aid in self.id_to_filename]
                assert len(accomp_ids) > 0, f"No valid accomp stems for {stem_id}"
                accomp_id = random.choice(accomp_ids)
                accomp_fn = self.id_to_filename[accomp_id]
                accomp_ext = accomp_fn.split(".")[-1]
                if accomp_ext == "pt":
                    accomp_latents = torch.load(accomp_fn)
                else:
                    accomp_latents = torch.from_numpy(np.load(accomp_fn))
                # Apply same crop window. If accomp is shorter than target window, zero-pad to match.
                target_len = latents.shape[-1]
                cropped = accomp_latents[:, start:start + target_len]
                if cropped.shape[-1] < target_len:
                    pad = torch.zeros(cropped.shape[0], target_len - cropped.shape[-1], dtype=cropped.dtype)
                    cropped = torch.cat([cropped, pad], dim=-1)
                info["accomp_input"] = cropped
                info["accomp_id"] = accomp_id
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    if dataset_type == "audio_dir":
        ...
        train_set = PreEncodedDataset(
            ...
            stem_mapping_path=dataset_config.get("stem_mapping_path", None),
            silence_threshold=dataset_config.get("silence_threshold", None),
        )
