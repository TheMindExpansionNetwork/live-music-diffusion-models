    ...
    if model_type == 'autoencoder':
        ...
        if "arc" in training_config:
            ...
            if type(teacher_model_config) == str:
                # its a path, load it!
                with open(teacher_model_config) as f:
            ...
            if teacher_model_config is not None:
                ...
                # assert that teacher model weights actually loaded
                for name, param in teacher_model.model.named_parameters():
                    print(f"Teacher model param: {name}, mean: {param.mean().item()}, std: {param.std().item()}")
                    break
                if teacher_model_ckpt is not None:
                    copy_state_dict(teacher_model, load_ckpt_state_dict(teacher_model_ckpt))
                else:
                    raise ValueError("teacher_model_ckpt must be specified if teacher_model is specified")
                del teacher_model.pretransform
                # assert that teacher model weights actually loaded
                for name, param in teacher_model.model.named_parameters():
                    print(f"Teacher model param: {name}, mean: {param.mean().item()}, std: {param.std().item()}")
                    break
            ...
            print("Discriminator model config:", discriminator_model_config)
            if discriminator_model_config is not None:
                ...
                if discriminator_model_ckpt is not None:
                    if discriminator_model_ckpt.endswith(".safetensors"):
                        state_dict = load_file(discriminator_model_ckpt)
                    else:
                        state_dict = torch.load(discriminator_model_ckpt, map_location="cpu", weights_only=True)["state_dict"]
            ...
            wrapper_kwargs = dict(
                ...
                accomp_kwargs=training_config.get("accomp_kwargs", None),
                context_router=training_config.get("context_router", False),
            )
