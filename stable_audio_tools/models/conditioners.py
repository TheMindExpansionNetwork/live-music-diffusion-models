...
def mix_t5_prompts(
    ...
    weighted_prompts: tp.List[tp.List[tp.Tuple[str, float]]],
    device: tp.Union[torch.device, str],
    weight_tol: float = 1e-4,
) -> tp.Tuple[torch.Tensor, torch.Tensor]:
    """Encode per-batch lists of (prompt, weight) pairs and weighted-sum them.

    Per batch item, weights MUST sum to 1.0 (within `weight_tol`). Embeddings are
    produced at the conditioner's fixed `max_length` (all prompts pad to the same
    length), so the mix is a simple weighted sum over per-token features. Attention
    masks are OR-unioned across prompts so cross-attn sees any position that was
    attended to by any prompt.
    """
    flat_prompts: tp.List[str] = []
    flat_weights: tp.List[float] = []
    group_counts: tp.List[int] = []
    for batch_idx, items in enumerate(weighted_prompts):
        assert len(items) > 0, f"weighted_prompts[{batch_idx}] is empty"
        w_sum = float(sum(w for _, w in items))
        assert abs(w_sum - 1.0) < weight_tol, (
            f"weighted_prompts[{batch_idx}] weights must sum to 1.0, got {w_sum}"
        )
        group_counts.append(len(items))
        for p, w in items:
            flat_prompts.append(p)
            flat_weights.append(float(w))

    embeddings, attention_mask = T5Conditioner.forward(t5_conditioner, flat_prompts, device)

    weights_tensor = torch.tensor(flat_weights, device=embeddings.device, dtype=embeddings.dtype)

    out_embeds = []
    out_masks = []
    offset = 0
    for n in group_counts:
        chunk_emb = embeddings[offset:offset + n]
        chunk_mask = attention_mask[offset:offset + n]
        chunk_w = weights_tensor[offset:offset + n].view(n, 1, 1)
        mixed_emb = (chunk_emb * chunk_w).sum(dim=0)
        mixed_mask = chunk_mask.any(dim=0)
        out_embeds.append(mixed_emb)
        out_masks.append(mixed_mask)
        offset += n

    return torch.stack(out_embeds, dim=0), torch.stack(out_masks, dim=0)


class MultiPromptT5Conditioner(T5Conditioner):
    """T5 conditioner that accepts a list of weighted prompts per batch element.

    Input: `List[List[Tuple[str, float]]]` — per-batch-element list of
    (prompt, weight) pairs. Weights per batch element must sum to 1.0.
    Output matches T5Conditioner: (embeddings, attention_mask), with embeddings
    being the weighted sum across prompts and attention_mask the OR-union.
    """
    ...
    def forward(
        ...
        return mix_t5_prompts(self, weighted_prompts, device)
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any], pretransform=None) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "t5_multi":
            conditioners[id] = MultiPromptT5Conditioner(**conditioner_config)
